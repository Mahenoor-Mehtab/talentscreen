...
function SessionPage() {
  ...
  const isHost = session?.host?.clerkUserId === user?.id;
  const isParticipant = session?.participant?.clerkUserId === user?.id;
  ...
  // Auto-join session for non-host users
  useEffect(() => {
    if (!session || !user || loadingSession || isHost || isParticipant) return;
    joinSessionMutation.mutate(id, { onSuccess: refetch });
  }, [session, user, loadingSession, isHost, isParticipant, id]);

  // Redirect both host and participant when session ends
useEffect(() => {
